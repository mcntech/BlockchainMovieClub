...
function findItemForMovieId(items, movieId) {
  for (let i = 0; i < items.size; i++) {
    if (items.get(i).item.movieId == movieId) return items.get(i).item;
  }
}

function findAbiIdForFunction(abi, fnName) {
  for (let i = 0; i < abi.length; i++) {
    if (abi[i].type == 'function' && abi[i].name == fnName) return i;
  }
  return -1;
}
...
class Player extends Component {
  constructor(props) {
    ...
    this.handleSubmitPurchaseDeferred = this.handleSubmitPurchaseDeferred.bind(
      this,
    );
    ...
    if (event == 'eth_call_response') {
      ...
      if (cmd == 'getMovieData') {
        try {
          ...
          const movieUrl = outputs[0];
          let movieThumb = outputs[1];
          const movieTitle = outputs[2];
          const movieDescript = outputs[3];

          const moviePrice = outputs[4];
          const movieDuration = outputs[5];
          const movieDrmType = outputs[6];
          const movieDrmStatus = outputs[7];

          // / Obtain thumb url from relative or absolute path
          if (
            movieThumb.indexOf('http://') === 0 ||
            movieThumb.indexOf('https://') === 0
          ) {
            // do nothing
          } else {
            movieThumb = movieUrl
              .substr(0, movieUrl.lastIndexOf('/') + 1)
              .concat(movieThumb);
          }
        ...
        try {
          ...
          var outputs = coder.decodeParameters(bcmc.abi[fnId].outputs, result);
          const _movieid = outputs[0];
          const _cgms = outputs[1];
          const _drmToken = outputs[2];
          console.log(
            `movieid:${_movieid} cgms:${_cgms} drm:${_drmToken} privateKey: ${privateKey.toString(
              'hex',
            )}`,
          );
          decryptWithPrivateKey(privateKey, JSON.parse(_drmToken)).then(
            _drmDecToken => {
              const item = findItemForMovieId(obj.props.items, _movieid);
              if (item != null) {
                console.log('_drmDecToken');
                console.log(_drmDecToken);
                const drmDecToken = JSON.parse(_drmDecToken);
        ...
        const id = findAbiIdForFunction(bcmc.abi, 'totalMovies');
  ...
  getTotalMovies() {
    ...
    const request = { cmd: 'totalMovies', calldata: callObj };
    this.ethlite.sendEthCallRequest(JSON.stringify(request));
  }

  getMovie(index) {
    const id = findAbiIdForFunction(bcmc.abi, 'getMovieData');
    const movieIndex = index;
    console.log(bcmc.abi[id]);
    const codedCall = coder.encodeFunctionCall(bcmc.abi[id], [
      movieIndex.toString(16),
    ]);
    console.log(`codedCall:${codedCall}`);
    ...
    const request = { cmd: 'getMovieData', calldata: callObj };
    this.ethlite.sendEthCallRequest(JSON.stringify(request));
  }

  getMovieDrm(index) {
    const id = findAbiIdForFunction(bcmc.abi, 'getMovieDrm');
    const movieIndex = index;
    console.log(bcmc.abi[id]);
    const codedCall = coder.encodeFunctionCall(bcmc.abi[id], [
      movieIndex.toString(16),
    ...
    const request = { cmd: 'getMovieDrm', calldata: callObj };
    this.ethlite.sendEthCallRequest(JSON.stringify(request));
  }
  updateMovieList() {
    this.movieReqIndex = 0;
    for (let i = 0; i < this.state.totalMovies; i++) {
      this.getMovie(i);
    }
  }
  ...
  handleSubmitPlayerAccount(event) {
    event.preventDefault();
    this.ethlite.sendEthGetAccount(
      `0x${this.state.playerAccount.toString('hex')}`,
    );
    this.deferredEthTransaction = this.handleSubmitPlayerAccountDeferred;
  }

  handleSubmitPlayerAccountDeferred(account_nonce) {
    const fnId = findAbiIdForFunction(bcmc.abi, 'registerPlayer');
    console.log(bcmc.abi[fnId]);
    const publicKeyStr = publicKeyOfPrivateKey(privateKey.toString('hex'));
    console.log(`publickey:${publicKeyStr}`);
    const codedCall = coder.encodeFunctionCall(bcmc.abi[fnId], [
      '0x00',
      '0x00',
      publicKeyStr,
    ]);
  ...
  handleSubmitPurchaseDeferred(account_nonce) {
    const fnId = findAbiIdForFunction(bcmc.abi, 'purchaseMovie');
    console.log(bcmc.abi[fnId]);
    const codedCall = coder.encodeFunctionCall(bcmc.abi[fnId], [
      this.deferredParam1.toString(16),
    ]);
    console.log(`codedCall:${codedCall}`);
  ...
  onMovieSelection(movieId) {
    const item = findItemForMovieId(this.props.items, movieId);
    if (item != null) {
      console.log(
        `onMovieSelection${movieId} url=${item.url} drmtype=${item.drmtype}`,
      );
        ...
        <div className={s.root}>
          <div className={s.container}>
            <div className="Player">
              <header className="Player-header">
                <h2 className="Player-title">
                  Demo Player ({this.state.EthState})
                </h2>
              </header>
            ...
            <div className="app-card-list" id="app-card-list">
              {tmp.map((item, key) => (
                <Card key={key} index={key} details={item} />
              ))}
            </div>
            <br />
  ...
  render() {
    if (this.props.enable) {
      return (
        <Context.Consumer>
          {context => (
            <button onClick={() => context.stopMovie()}> Stop</button>
          )}
        </Context.Consumer>
      );
    }
      ...
      <Context.Consumer>
        {context => (
          <button
            ...
            onClick={() => {
              this.fireClick(itemId);
              context.onMovieSelection(itemId);
            }}
          >
            <i className="fa fa-chevron-right" /> Select Movie
          </button>
        )}
      </Context.Consumer>
    );
      ...
      <Context.Consumer>
        {context => (
          <button
            className="button button-primary"
            onClick={() => {
              this.fireClick(itemId, price);
              context.onMoviePurchase(itemId, price);
            }}
          >
            <i className="fa fa-chevron-right" /> Purchase
          </button>
        )}
      </Context.Consumer>
    );
    ...
    return (
      <div className="card-body">
        ...
        <p className="body-content">
          {' '}
          Duration: {this.props.details.duration} Price:
          {this.props.details.price} DRM:{this.props.details.drmtype} DRM
          Status:{this.props.details.drmstatus}
        </p>
        <ButtonSelect itemId={this.props.details.movieId} />
        <ButtonPurchase
          itemId={this.props.details.movieId}
